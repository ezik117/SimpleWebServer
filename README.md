...
Язык шаблонов состоит из *переменных* и *команд*. Первые задаются через двойные фигурные скобки `{{ <имя переменной> }}`, вторые через комбинацию двойной скобки и знака процента <!--{% raw %}-->`{% <команда> %}`<!--{% endraw %}-->. Синтаксис управляющего кода языка шаблонов следующий:
